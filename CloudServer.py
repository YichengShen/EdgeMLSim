...
class CloudServer:
    ...
    def process(self):
        ...
        threading.Thread(target=self.send_model_to_simulator,
        ...
        while True:
            ...
            with self.cv:
                self.cv.wait_for(lambda: self.terminated or config_ml.cloud_aggregation_condition(
                    self.accumulative_gradients))
    ...
    def update_model(self):
        ...
        aggregated_nd = config_ml.aggre(
            gradients_to_aggregate, byz=config_ml.BYZ_TYPE_CLOUD)
        ...
        for param in self.model.collect_params().values():
            if param.grad_req != 'null':
                ...
                param.set_data(param.data(
                ) - lr * aggregated_nd[idx:(idx+param.data().size)].reshape(param.data().shape))
                idx += param.data().size
    ...
    def send_parameter(self):
        for conn in self.connections:
            send_message(conn, InstanceType.CLOUD_SERVER,
                         PayloadType.PARAMETER, self.parameter)
    ...
    def send_model_to_simulator(self, simulator_conn):
        ...
        while not self.terminated:
            ...
            if model_request_msg.get_payload_type() == PayloadType.REQUEST:
                send_message(simulator_conn, InstanceType.CLOUD_SERVER,
                             PayloadType.MODEL, self.model)
            else:
